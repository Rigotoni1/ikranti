...
export default function AdminConsole(){
 ...
 const approvalChecks=current&&section!=="sellers"?[
...
{!rows.length?<p className="muted">{section==="pending"?"You're up to date. No listings awaiting approval.":"No records yet."}</p>:<ul className="adminRecords">{rows.map(row=><li key={String(row.id)}>{section!=="sellers"&&<ListingPhoto key={String(row.image_path)} row={row}/>}<div><strong>{String(section==="sellers"?row.name:row.title)}</strong><span>{section==="sellers"?(applicationName(row.id)):String(row.category)+" · "+sellerName(row.seller_id)}</span><span className="adminBadge">{row.suspended?"Suspended":label(section==="sellers"?row.seller_status:row.status)}</span></div><button aria-pressed={selected===row.id} onClick={()=>setSelected(String(row.id))}>{section==="sellers"?"View seller":"Review listing"} →</button></li>)}</ul>}
</section>
...
{section!=="sellers"&&<><ListingPhoto key={String(current.id)+String(current.image_path)} row={current} large/><p>{String(current.description)}</p><dl>{["category","location","start_price","reserve_price","end_at","review_note"].map(key=><div key={key}><dt>{label(key)}</dt><dd>{String(current[key]??"—")}</dd></div>)}</dl><p>Submitted by {sellerName(owner)}</p></>}
...
{section!=="sellers"&&current.status==="under_review"&&<div className="adminChecklist"><h3>Approval checklist</h3><ul>{approvalChecks.map(check=><li key={check.title}><span>{check.ok?"✓ Ready":"Missing"}</span> {check.title}</li>)}</ul>{!canApprove&&<p>The seller must add the missing information before this listing can go live.</p>}</div>}
{message&&<p className="portalMessage" role="status">{message}</p>}
<form key={String(current.id)+section} onSubmit={e=>{e.preventDefault();const data=new FormData(e.currentTarget);const submitter=(e.nativeEvent as SubmitEvent).submitter as HTMLButtonElement|null;if(submitter?.name==="action")data.set("action",submitter.value);void run(async()=>{await rpc("ir_admin_action",{p_action:field(data,"action"),p_target:String(current.id),p_note:field(data,"note"),p_fingerprint:field(data,"fingerprint")||null});},"Decision saved");}}>
