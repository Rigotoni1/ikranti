...
export default function AdminConsole(){
 ...
 const directoryRows=rows.filter(row=>(!search.trim()||`${row.name} ${row.email}`.toLowerCase().includes(search.trim().toLowerCase()))&&(!needsReview||(!row.identity_approved&&row.identity_submitted)||row.seller_status==="pending"));
...
{accountSection?<><div className="directoryFilters"><label>Find a user<input type="search" value={search} placeholder="Search name or email" onChange={e=>setSearch(e.target.value)}/></label><label className="checkLabel"><input type="checkbox" checked={needsReview} onChange={e=>setNeedsReview(e.target.checked)}/>Needs verification / seller review</label></div><div className="userDirectory"><table><thead><tr>{["Name","Roles","Email verified","Identity status","Account status","Latest staff decision","Manage"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{directoryRows.map(row=><tr key={String(row.id)}><td data-label="Name"><strong>{String(row.name||"Member")}</strong><small>{String(row.email||"")}</small><small>Joined {String(row.created_at).slice(0,10)}</small></td><td data-label="Roles">{roles(row)}</td><td data-label="Email verified">{row.email_verified?"Yes":"No"}</td><td data-label="Identity status">{identity(row)}<small>Buyer setup: {row.buyer_complete?"Complete":"Incomplete"} · Seller: {label(row.seller_status)}</small></td><td data-label="Account status">{row.suspended?"Suspended":"Active"}</td><td data-label="Latest staff decision">{row.last_staff_decision?<>{decisionLabels[String((row.last_staff_decision as Row).action)]||label((row.last_staff_decision as Row).action)}<small>{new Date(String((row.last_staff_decision as Row).created_at)).toLocaleDateString("en-MT")}</small></>:"No decision yet"}</td><td><div className="directoryActions"><button disabled={busy} onClick={()=>select(String(row.id))} aria-pressed={selected===row.id}>View profile</button><button disabled={busy} onClick={()=>select(String(row.id),"Staff decisions")}>Staff decisions →</button></div></td></tr>)}</tbody></table>{!directoryRows.length&&<p>No users match this view. Try clearing the search or review filter.</p>}</div></>:!rows.length?<p className="muted">{section==="pending"?"You're up to date. No listings awaiting approval.":"No records yet."}</p>:<ul className="adminRecords">{rows.map(row=><li key={String(row.id)}>{!accountSection&&<ListingPhoto key={String(row.image_path)} row={row}/>}<div><strong>{String(accountSection?row.name:row.title)}</strong><span>{String(row.category)+" · "+sellerName(row.seller_id)}</span><span className="adminBadge">{label(row.status)}</span></div><button aria-pressed={selected===row.id} onClick={()=>select(String(row.id))}>{"Review listing"} →</button></li>)}</ul>}
</section>
...
{!accountSection&&current.status==="under_review"&&<form key={String(current.id)+section} onSubmit={e=>{e.preventDefault();const data=new FormData(e.currentTarget);const submitter=(e.nativeEvent as SubmitEvent).submitter as HTMLButtonElement|null;if(submitter?.name==="action")data.set("action",submitter.value);void run(async()=>{await rpc("ir_admin_action",{p_action:field(data,"action"),p_target:String(current.id),p_note:field(data,"note")});setReviewed(true);},"Decision saved. The seller has been notified in their account.");}}>
