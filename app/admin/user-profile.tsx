...
export function UserProfile({user,users,download,busy,onBusy,onSaved,initialTab="Overview"}:{user:Row;users:Row[];download:(path:string)=>void;busy:boolean;onBusy:(busy:boolean)=>void;onSaved:()=>Promise<void>;initialTab?:string}){
 ...
 <nav className="portalTabs" aria-label="User profile sections">{["Overview","Verification","Staff decisions","Listings","Orders","Activity"].map(t=><button key={t} disabled={busy} aria-pressed={tab===t} className={tab===t?"active":""} onClick={()=>setTab(t)}>{t}</button>)}</nav>
 ...
 {detail.onboarding.map(o=><section key={String(o.account_type)}><h3>{text(o.account_type)} details</h3><dl>{Object.entries((o.details||{}) as Row).map(([k,v])=><div key={k}><dt>{text(k)}</dt><dd>{typeof v==="boolean"?(v?"Yes":"No"):Array.isArray(v)?v.map(text).join(", "):text(v)}</dd></div>)}</dl></section>)}</>}
 ...
 <div hidden={tab!=="Staff decisions"}><StaffDecisions user={user} documents={detail.documents} onboarding={detail.onboarding} application={detail.application} busy={busy} onBusy={onBusy} onSaved={onSaved}/></div>
